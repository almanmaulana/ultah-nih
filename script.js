...
async function mulainama() {
  ...
  // Sembunyikan dulu stikernya
  fotostiker.style = "display:none;";

  // 1. Buat container bingkai untuk foto kejutan
  ...
  teksAtas.style = `
    ...
    text-align: center;
  `;
  bingkai.appendChild(teksAtas);

  // 3. Tambah gambar kejutan
  const imgKejutan = document.createElement("img");
  imgKejutan.src = "https://raw.githubusercontent.com/almanmaulana/foto/refs/heads/main/ini.png";
  imgKejutan.alt = "Foto Kejutan";
  imgKejutan.style = `
    width: 100%;
    border-radius: 15px;
    object-fit: cover;
  `;
  bingkai.appendChild(imgKejutan);

  // 4. Tampilkan di halaman
  document.getElementById("Content").prepend(bingkai);

  // 5. Setelah 5 detik, hilangkan bingkai perlahan
  setTimeout(() => {
    bingkai.style.transition = "opacity 0.5s ease";
    bingkai.style.opacity = "0";
